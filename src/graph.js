...
class Graph {
  ...
  contains(value) {
    let i = 0;
  // ---------------------------------------------------------------
    for (; i < this.vertices.length; i++) { // this was really easy loop through the list and find value
  ...
  removeVertex(value) {
    ...
    for (; i < this.vertices.length; i++) {
      if (this.vertices[i].value === value) reVertex = this.vertices.splice(i, 1);
    }
    // -------------------------------------------------------------
    if (!reVertex) {
      i = 0;
      // ----------------------------------------------------------- 
      for (; i < reVertex.numberOfEdges; i++) {
        this.removeEdge(reVertex, reVertex.edges[i]);
      }
      // -----------------------------------------------------------
    }
    // -------------------------------------------------------------
