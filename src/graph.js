...
class Graph {
  ...
  addVertex(value, edges = []) {
    ...
    // connects back to this new node we're creating
    if (edges.length > 0) {
      edges.forEach((node) => {
        this.addEdge(newNode, node);
      });
    }
  ...
  contains(value) {
    return this.vertices.findIndex((node) => node.value === value) !== -1;
  }
  ...
  removeVertex(value) {
    ...
    if (index !== -1) {
      this.vertices[index].edges.forEach(node => this.removeEdge(this.vertices[index], node));
      this.vertices.splice(index, 1);
      // const removedVertex = this.vertices.splice(index, 1)[0];
      // removedVertex.edges.forEach(node => this.removeEdge(removedVertex, node));
    }
  }
  ...
  removeEdge(fromVertex, toVertex) {
    if (this.checkIfEdgeExists(fromVertex, toVertex)) {
      fromVertex.edges = fromVertex.edges.filter(x => x.value !== toVertex.value);
      toVertex.edges = toVertex.edges.filter(x => x.value !== fromVertex.value);
    ...
    } // The problem here was that we weren't adding in .value into anything (toVertex instead of toVertex.value).
  }
}
