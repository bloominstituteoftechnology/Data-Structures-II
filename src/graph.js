...
class Graph {
  ...
  addVertex(value, edges = []) {
    ...
    if (edges.length) {
      edges.forEach(e => this.addEdge(newVert, e));
    }
  ...
  removeVertex(value) {
    ...
    if (!vertex) return;
    vertex.edges.forEach(v => this.removeEdge(vertex, v));
  ...
  removeEdge(fromVertex, toVertex) {
    const to = toVertex.edges.indexOf(fromVertex);
    const from = fromVertex.edges.indexOf(toVertex);
    toVertex.edges.splice(to, 1);
    fromVertex.edges.splice(from, 1);
